...
> **实现说明（后补）**：本报告记录的是改造前的实现 —— 主权重存成 MXFP8、
> 无 fp32 副本、靠随机舍入更新，动量用 bf16，MXFP8 recipe 是纯 E4M3。
> 精度方案此后按低精度训练规范做了调整（主权重与优化器状态改为 fp32、
> GEMM 换成 HYBRID、两条腿的初始化对齐），详见
> [`docs/06`](../docs/06-低精度训练.md)。本报告的**测量过程与数据本身不受影响**，
> 但其中的精度描述与当前代码不一致；§5 的 A/B 结论另见下方说明。
...
**随机舍入**写回。踩过的五个坑与设计取舍见 [`docs/06`](../docs/06-低精度训练.md)，
这里只列与本次运行直接相关的三条：
...
有一边不匹配，见 [`docs/06` 第五条](../docs/06-低精度训练.md)。）
...
> ⚠️ **后补的重要限定：这一节测的不是「FP8 vs BF16」，而是三个变量的合力。**
> 事后核查发现两条腿除 FP8 之外还有两处不同：优化器动量精度（bf16 vs fp32），
> 以及**初始权重** —— 同一 seed 下 126 个参数张量里有 75 个逐位不同，
> 因为 `te.Linear` 的默认初始化与 `nn.Linear` 分布不同、消耗的 RNG 流也不同。
> 超参、种子、卡数都锁死了，但**锁住种子不等于锁住初值**。
> 下面的胜负数据本身是可靠的，只是不能归因给精度。三处差异现已全部消除，
> 这个对照需要重跑，见 [`docs/08`](../docs/08-已知问题与后续.md)。
...
| 相关文档 | |
|---|---|
| FP8 主权重的设计与踩坑 | [`docs/06-低精度训练.md`](../docs/06-低精度训练.md) |
