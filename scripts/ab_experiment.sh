...
shape_for() {
  case "$1" in
    ...
    # `polh-*` = 给策略头加一层隐藏层（512 宽 + SiLU）。参数 +1.1%。
    # 它是全网唯一没有非线性的头，而输出是 17836 维：每个动作摊 1.31 个参数，
    # 而价值头每个输出摊 44,033 个。低模拟数下搜索只看先验排前 16 的着法，
    # 先验就近乎直接决定落子 —— 却来自最单薄的那个头。
    polh-*)  echo "--policy-hidden 512" ;;
    # `mobp-*` = 多吃两个「可达度」输入平面（每格被己方/对方多少个合法着法覆盖）。
    # 前六个结构实验改的都是「容量怎么排布」「学什么任务」，没有一条给过网络
    # 新的信息 —— 这是仅剩的一类。可达度正是 greedy-mobility 赖以取胜的那个量，
    # 网络本来要从「21 枚在手棋子 x 局部形状匹配」里自己推，而引擎白送。
    mobp-*)  echo "--in-planes 11" ;;
    # `data4-*` = 每轮自博弈局数 x4（2048 -> 8192），训练步数不变。
    #
    # 实测样本复用率是 **6.7x**：每轮新增 61,433 个局面，却抽样 409,600 次
    # （400 步 x 1024）—— 每个自博弈局面平均被训练 6.7 次。x4 之后降到 1.7x。
    #
    # 这是七个结构实验之后剩下的两个方向之一。支持它的证据：**每一次加容量
    # 都变差**（加宽 -24、每层注意力 -26、445M 骨干 -106），这正是数据不足
    # 相对模型容量的经典症状 —— 模型有余力，而它只是在反复拟合同一批自产数据。
    #
    ...
    # `data8-*` = 每轮 16384 局，复用率 0.85x（每个新局面平均被抽不到一次）。
    # 4x 实测 +85（对尺子），2x 是 -13.9 —— 这条轴非单调，8x 必须实测，不能外推。
    data8-*) echo "--games-per-iter 16384" ;;
    *)      echo "" ;;
  esac
}
...
budget_for() {
  case "$1" in
    ...
    *-x190) echo "--total-steps 190000 --lr-horizon-steps 220000 --lr-decay-steps 20000" ;;
    # `-x130` = 平顶跑到 13 万就停（horizon 15 万、退火 2 万 -> 退火点正在 13 万）。
    # 给 8x 用：8x 的自博弈是 4x 的两倍，2.2 步/s，19 万步要 24 小时。
    # 而 4x 的曲线已经给出答案 —— **峰值在 14.4 万，18.9 万与 21.1 万都不再涨**，
    # 所以 8x 只要能分叉出 a144 就够比了，平顶到 13 万即可（a144 从 13 万退火）。
    # 顺带留下 10 万的里程碑，需要时还能补一个 a111。
    *-x130) echo "--total-steps 130000 --lr-horizon-steps 150000 --lr-decay-steps 20000" ;;
