...
def evaluate_vs_baseline(
    ...
    `flat-mcts-4k` 是 991.6 Elo，而网络第 1 万步就到 980、峰值在 1500 以上 ——
    峰值网络对整条阶梯的期望得分率只从 0.9988（random）走到 0.9742（flat-mcts-4k），
    整段宽 0.0246，而 200 局在这一带的标准误就有 ±0.011，噪声比信号还宽。

    实际后果比「没分辨率」更糟：它会在真实棋力回落的那一段继续给出「越来越好」。
    恒向上的指标比没有指标更危险。棋力判断改为事后用 `tools/net_arena.py` 做
    （那才给出了正确的峰值）。
    """
